...
TwoMelodyChordRandomGenerator::TwoMelodyChordRandomGenerator(Type * type, MIDITrack * target) : SongGenerator(type, target) {
	this->trackWork = new TrackBoard(target);
}

void TwoMelodyChordRandomGenerator::generateTrack() {
	...
	MelodyParameter<std::vector<std::string> *> * midModulatorParam = midParams->setParam<std::vector<std::string> *>(MelodyParameterType::MODULATOR,
		new std::vector<std::string>({ "ChordNoteModulation", "RandomShiftNoteModulation" })
	);
