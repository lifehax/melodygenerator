...
void PresetGeneratorOptions::init() {
	...
	this->optionArea = CreateWindowEx(NULL, L"GeneratorOptionsArea", (LPCTSTR)NULL, WS_CHILD | WS_BORDER | WS_VSCROLL,
	...
	this->optionDefaultText = CreateWindow(WC_STATIC, L"Select Generator Type On Left To Continue...", WS_CHILD | SS_LEFT,
		0, 0, 300, 20,
		this->optionArea, (HMENU)9007, NULL, NULL);

	this->noOptionsText = CreateWindow(WC_STATIC, L"No Options For Type, Click Generator To Run", WS_CHILD | SS_LEFT,
		0, 0, 300, 20,
			...
			for (auto checkOption : *checkOptionType.second) {
				if (checkOption.second->getRequired()) {
					if (checkOption.second->getValue() == "") {
						MessageBox(this->mainWindow->getWindow(), L"Missing Generator Option Value", NULL, MB_APPLMODAL);
						return false;
					}
				}
			}
